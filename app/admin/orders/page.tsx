  ...
  return (
    <div className="bg-[#050505] min-h-screen text-white font-sans pb-20 selection:bg-emerald-500 selection:text-black">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 mt-6 md:mt-10">
        ...
        {/* LISTA GENERAL DE ÓRDENES RESTAURADA CON SCROLL HORIZONTAL ANTI-ROTURA EN MÓVILES */}
                ...
                <div className="min-w-[800px] md:min-w-0 grid gap-4 md:gap-5">
                    {orders.map((order) => {
                      ...
                      return (
                        <Link 
                          key={order.id} 
                          href={`/admin/orders/${order.order_id}`} // ✅ LLEVA A LA PANTALLA INDIVIDUAL
                          className="group bg-[#0a0a0c] backdrop-blur-xl border border-zinc-800/80 p-5 md:p-8 rounded-[2rem] hover:border-emerald-500/50 hover:bg-zinc-900/60 transition-all duration-300 flex flex-row justify-between items-center gap-6 shadow-xl"
                        >
      ...
      {/* --- MODAL REPORTE DE DIRECTORIO (RESTAURADO Y OPTIMIZADO) --- */}
      {showBoardReport && (
      ...
      {/* --- MODAL NUEVO ATLETA (RESTAURADO Y OPTIMIZADO) --- */}
      {showModal && (
